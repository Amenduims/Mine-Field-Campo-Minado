...
import static com.example.jogominado2.R.drawable.*;
import static com.example.jogominado2.R.drawable.bandeirinha;
import static com.example.jogominado2.R.drawable.bandeirinha3;
...
public class CustomMode extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        CustomMode.this.getWindowManager().getDefaultDisplay().getMetrics(dm);

        tamLines = getIntent().getExtras().getInt("tamLines");
        tamColumn = getIntent().getExtras().getInt("tamColumn");
        ...
        for (int i = 0; i < tamLines; i++) {
            for (int j = 0; j < tamColumn; j++) {
                ...
                if(btn[i][j].getText().equals("B")) {
                    ...
                    btn[i][j].setIconGravity(MaterialButton.ICON_GRAVITY_TEXT_START);
                    Toast.makeText(CustomMode.this, "Perdeu!!!", Toast.LENGTH_SHORT).show();
                    perdeu = true;
                    liberarTudo();
                }
